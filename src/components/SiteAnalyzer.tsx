...
export const SiteAnalyzer = () => {
  ...
  const checkLinks = async () => {
    ...
    toast({
      ...
      description: `Found ${broken.length} broken links and ${redundant.length} redundant routes`,
      duration: 5000,
    });
  };
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto mt-8">
      ...
      <CardContent>
        ...
        {brokenLinks.length === 0 && redundantRoutes.length === 0 && !isChecking && (
          <Alert>
            <AlertDescription className="text-green-600">
              No issues found! All links are valid and no redundant routes detected.
            </AlertDescription>
          </Alert>
        )}
      </CardContent>
